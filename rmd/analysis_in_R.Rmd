...
LeagueResults <- function(league){
    #' Calculate results and table for given national league.

    teams <- sort(unique(league$team))

    home_score <- matrix(0, nrow=length(teams), ncol=length(teams))
...
```{r}
prem <- LeagueResults(events)
```

```{r}
pl <- as.data.frame(prem)
```
...
```{r}
NationalResults <- function(nation) {
 
    data <- LoadAndScaleData(nation)
    results <- LeagueResults(data)
    return(results)
}
...
```{r}
correct_res <- read.csv('../data/league_results/England17-18.csv')
```
...
```{r}
france <- NationalResults('France')
germany <- NationalResults('Germany')
italy <- NationalResults('Italy')
spain <- NationalResults('Spain')
```

```{r}
premiership <- NationalResults('England')
```
...
```{r}
PassNetwork(events, match_id, team_colour='red')
PassNetwork(events, match_id, team_colour='blue', home_or_away='Away', flip=T)
```
...
#     transition_matrix <- TransitionMatrix(GameGraph(events, match_id))
    ...
    for (i in (1:(ncol(transition_matrix)-2))){
        ...
        state_probabilities <- matrix(NA,nrow=max_steps+1,ncol=ncol(transition_matrix),
...
MarkovExpectedGoals <- function(events, season_adjacency_matrix) {
    ...
    normalize <- seasonMXG/(mean(seasonMXG))
    avg_events_per_game <- nrow(events)/380
    scale <- normalize * avg_events_per_game - avg_events_per_game
    sorted <- sort(scale, decreasing=T)   
...
```{r}
GoalProbabilities <- function(season_adjacency_matrix) {
    
    season_graph <- graph_from_adjacency_matrix(season_adjacency_matrix)
    season_transition <- TransitionMatrix(season_graph)
    goal_probs <- sort(season_transition[,'Goal'],T)[2:nrow(season_transition)] * 100
...
leagueMXG <- function(events){
    ...
    for (team in teams){
        ...
        season_adjacency_matrix <- SeasonAdjacencyMatrix(events, team)
        mXG <- MarkovExpectedGoals(events, season_adjacency_matrix)

        all_mXG <- append(all_mXG, mXG)     
...
for (league in leagues){
    ...
    mXG <- append(mXG, leagueMXG(events))
}
```

```{r}
mXG_df <- data.frame(sort(mXG,T))
```
...
```{r}
write.csv(mXG_df,'../data/processed/mXG.csv')
```
...
```{r}
rownames(mXG_df) <- names(sort(mXG,T)[1:20])
```

```{r}
name_list <- names(sort(mXG,T))
```
...
```{r}
g <- GameGraph(events, match_id, home_or_away='Home')
```
