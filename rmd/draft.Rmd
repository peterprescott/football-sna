...
Given an initial distribution $\pi_{0}$, we know that after $k$ transitions, the state $\pi_{k} = \pi_{0}P^{k}$. 

We can define the *state vector* $\delta$ of any particular state $s{i}$ as
$$
...
The probability $\mathbb{P}(X_{k}=s)$ is then given by the dot product $\langle \pi_{k}, \delta \rangle$.

If $s_{i}$ is an absorbing state, then the probability that it is reached *for the first time* is given by  $p_{k}:= \langle \pi_{k}, \delta \rangle - \langle \pi_{k-1}, \delta \rangle$. 

If the expectation of the hitting time is finite it can then be approximated directly by iterating up to some large $K$:
$$\mathbb{E}[\tau_{s}] = \sum_{k=0}^\infty kp_k \approx \sum_{k=0}^K kp_k$$ 

Thus we can quantify, for each player on the team, the expected number of events there will be before the team scores a goal. We invert this to give a measure of the expected number of goals; and scale it by the team's average number of events per game, and by the average player's expected goals values, and call this measure *Markov Expected Goals* (MXG).
