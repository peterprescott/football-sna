...
MarkovExpectedGoals <- function(events, season_adjacency_matrix) {
    ...
    mXG <- sort(avg_events_per_game/seasonXST, decreasing=T)   
...
```{r}
liverpool_adj <- SeasonAdjacencyMatrix(events,'Liverpool')

```

```{r}
mXG <- MarkovExpectedGoals(events, liverpool_adj)
```
...
```{r}
GoalProbabilities(liverpool_adj)
```

```{r}
sort(liverpool_adj[,'Goal'],T)
```
